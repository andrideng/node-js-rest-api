...
handlers.public = (data, callback) => {
    // Reject any request that isn't a GET
    if (data.method == 'get') {
        // Get the filename being requesttd
                                        ...
                                        _data.delete('checks', checkId, (err) => {
                                            ...
                                            if (checksDeleted == checksToDelete) {
                                                if (!deletionErros) {
                                                    ...
                                                    callback(500, {
                                                        Error: 'Errors encountered while attempting to delete all of the user\'s checks. All checks may not have been deleted from the system succesfully!'
                                                    });
                                                }
                                            }
                                        });
...
// Conatiner for all the tokens methods
handlers._tokens = {};
        ...
        _data.read('users', phone, (err, userData) => {
            if (!err && userData) {
                ...
                if (hashedPassword == userData.hashedPassword) {
                    // if valid, create a new token with a random name. Set expiration date 1 hours in the future
                            ...
                            if (!err) {
                                ...
                                _data.read('users', checkData.userPhone, (err, userData) => {
                                    if (!err && userData) {
                                        ...
                                        callback(500, {
                                            Error: 'Could not find the user who created the check, so sould not remove the user from list of user object!'
                                        });
                                    }
                                });
                            } else {
