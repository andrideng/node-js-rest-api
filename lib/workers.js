...
workers.validateCheckData = (originalCheckData) => {
    ...
    originalCheckData.protocol == typeof (originalCheckData.protocol) == 'string'
        && ['http', 'https'].indexOf(originalCheckData.protocol)
    ...
    originalCheckData.method == typeof (originalCheckData.method) == 'string'
        && ['get', 'post', 'put', 'delete'].indexOf(originalCheckData.method)
    ...
    originalCheckData.state == typeof (originalCheckData.state) == 'string'
        && ['up', 'down'].indexOf(originalCheckData.state)
    ...
    if (originalCheckData.id 
        ...
        debug('Error: One of the checks is not properly formatted. Skipped it.')
    }
};
...
workers.performCheck = (originalCheckData) => {
    ...
    const path = parseUrl.path; // Using path and not "pathname" because we want to use the query string.
...
// Process the check outcome, and update the check data as needed, trigger an alert to use if needed
// Special logic for accomodating a check that has never been tested before (don't need to alert to that one)
workers.processCheckoutOutcome = (originalCheckData, checkOutcome) => {
    ...
    _data.update('checks', newCheckData.id, newCheckData, (err) => {
        if (!err) {
            // Send the new check data to the nexst phase in the process if needed
            if (alertWarranted) {
