...
cli.responders.help = () => {
    const commands = {
        ...
        'list checks --up --down': 'Show a list of all the active checks in the sytem, including their state. The "--up" and the "--down" flags are both optional',
        'more check info --{checkId}': 'Show details of a specified check',
        'list logs': 'Show a list of all the logs files available to be read (compress and uncompressed)',
...
// Exit
cli.responders.exit = () => {
    console.log('Thanks, Please come back again!');
    process.exit(0);
};
            ...
            userIds.forEach((userId) => {
                _data.read('users', userId, (err, userData) => {
                    if (!err && userData) {
                        ...
                        let line = 'Name: ' + firstName + ' ' + lastName + ' Phone: ' + phone + ' Checks: ';
                        let numberOfChecks = typeof (checks) == 'object' && checks instanceof Array && checks.length > 0
                            ? checks.length
                            : 0;
                        line += numberOfChecks;
                        console.log(line);
...
cli.processInput = (str) => {
    ...
    if (str) {
        // Codify the unique string that identify the unique question allowed to be answer
        const uniqueInputs = [
            'man',
            'help',
            'exit',
